...
as $$
  ...
  bars as (
    select
      ...
      lag(b.close::numeric) over (
        partition by b.symbol
        order by b.bar_start_at
      ) as previous_close,
      row_number() over (
        partition by b.symbol
        order by b.bar_start_at desc
      ) as recent_rank
  ...
  select
    case
      when count(*) filter (where sma40_count = 40) = 0 then null
      else round(
        (
          count(*) filter (
            where sma40_count = 40
              and latest_close > sma40
          )::numeric
          / count(*) filter (where sma40_count = 40)::numeric
        ) * 100,
        2
      )
    end as t2108,
    count(*) filter (where sma40_count = 40)::integer as t2108_covered,
    count(*) filter (
      where anchor34_close > 0
        and ((latest_close - anchor34_close) / anchor34_close) * 100 >= 13
    )::integer as up_13_in_34_days,
    count(*) filter (
      where anchor34_close > 0
        and ((latest_close - anchor34_close) / anchor34_close) * 100 <= -13
    )::integer as down_13_in_34_days,
    case
  ...
  from symbol_metrics
  cross join sums
  group by sums.up5, sums.down5, sums.up10, sums.down10;
$$;
